            ...
            <div className=" flex flex-row p-3  w-full h-full space-x-5">
                {Exams.slice(0,5).map((exam) => (
                    <div key={exam.id} className="bg-[#335708] rounded-2xl w-full h-full flex flex-col">
                        <div className="text-[1.2rem] max-w-[1rem] font-semibold text-white pt-2 pb-4 pl-3">{exam.syntax}</div>
                        <div className=" w-full h-0.5 bg-[#86EE03]" />
                        <p className="w-full h-full text-[#86EE03] font-semibold text-[1.2rem] flex justify-end items-end pb-2  pr-2 "><span className="text-4xl pr-0.5">{exam.score}</span>/100</p>
                    </div>
